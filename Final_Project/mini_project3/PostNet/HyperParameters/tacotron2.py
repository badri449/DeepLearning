...
class Postnet(nn.Module):
    ...
    def __init__(self, in_out_channels, num_convs=4):
        super().__init__()
        self.convolutions = nn.ModuleList()
        self.convolutions.append(ConvBNBlock(in_out_channels, 256, kernel_size=3, activation="tanh"))

        for _ in range(1, num_convs - 1):
            self.convolutions.append(ConvBNBlock(256, 256, kernel_size=3, activation="tanh"))
        self.convolutions.append(ConvBNBlock(256, in_out_channels, kernel_size=3, activation=None))
    def forward(self, x):
        o = x
